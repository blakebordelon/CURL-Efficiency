...
if __name__ == '__main__':
    ...
    _, sup_acc = curltrainer.train(epochs=400, batch_size=5,  test_freq=1000)
    curltrainer.get_approximate_labels()
    _, sims, conts = curltrainer.curltrain(epochs=200, batch_size=5)
    _, postcurl_acc = curltrainer.train(epochs=300, batch_size=5,  test_freq=1000)

    supfile = f"CURL/SVHN/plots/test-sup-{run}-{labeledfrac}-{unlabeledfrac}.npy"
    unsupfile = f"CURL/SVHN/plots/test-unsup-{run}-{labeledfrac}-{unlabeledfrac}.npy"
    simfile = f"CURL/SVHN/plots/test-sim-{run}-{labeledfrac}-{unlabeledfrac}.npy"
    contfile = f"CURL/SVHN/plots/test-cont-{run}-{labeledfrac}-{unlabeledfrac}.npy"
